    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row>
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="primary" @click="shangjai(scope.row,0,'关闭')" v-if="scope.row.BSwitch==1">
            关闭（轮播图）
          </el-button>
          <!-- <el-button size="mini" v-if="scope.row.Status!=2" type="danger" @click="shangjai(scope.row,2,'解约')">
            解约
          </el-button> -->
          <el-button size="mini" v-if="scope.row.BSwitch==0" type="danger" @click="shangjai(scope.row,1,'开启')">
            开启（轮播图）
          </el-button>
        </template>
      </el-table-column>
